...
public:
  explicit Mutex() {
    ...
    TX_ASSERT_SYSCALL(pthread_mutexattr_settype(&attr, PTHREAD_MUTEX_RECURSIVE));
...
public:
  ...
  ~MutexGuard() {
    if (lock_) {
      // A NULL lock means that this MutexGuard has been moved to another
      // MutexGuard, in which case we must avoid a double-unlock on lock.
      lock_->unlock();
    }
  }
