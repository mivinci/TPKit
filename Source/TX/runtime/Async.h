...
public:
  ...
  T await_resume() { return handle_.promise().result().unwrap(); }
...
public:
  ...
  void return_value(T t) { result_ = std::move(t); }
  void unhandled_exception() { result_ = std::current_exception(); }

private:
  Result<T, std::exception_ptr> &result() { return result_; }

private:
  friend Async<T>;
  std::coroutine_handle<> continuation_;
  Result<T, std::exception_ptr> result_;
};
} // namespace TX
