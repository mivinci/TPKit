...
template <> struct fmt::formatter<TX::Logger::Level> : formatter<string_view> {
...
Logger::Log::Log(Level level, const char *file, int line, const char *function,
                 const char *scope, const char *format, Args &&...args)
    : level(level), line(line), file(file), scope(scope), function(function),
